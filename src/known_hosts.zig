//! `known_hosts` — reading and matching OpenSSH's host-key trust database.
//!
//! The library hands a `HostKeyInfo` to the caller and waits for
//! `acceptHostKey()` or `rejectHostKey()`. That split is deliberate: the trust
//! decision is policy, and policy belongs to the embedder. What does not
//! belong to the embedder is the parsing behind the decision -- OpenSSH's
//! `known_hosts` format has enough in it (host patterns, non-default ports,
//! `@revoked` and `@cert-authority` markers, key-blob well-formedness) that
//! every caller reimplementing it is every caller getting a different subset
//! of it right.
//!
//! So this is the reading, and none of the deciding.
//!
//! ## No I/O
//!
//! Nothing here opens a file. `checkContents` takes the bytes of a
...
//! ## What is refused
//!
//! A line whose host field carries a `@`-marker, a wildcard, or a hashed
//! (`|1|...`) pattern is an error rather than a non-match. Treating a
//! marker this module does not implement as "no entry" would turn a revoked
//! key into an unknown one, and prompting to trust a key the file explicitly
//! revokes is the one outcome worth ruling out by construction.
...
/// The `known_hosts` line that would trust `raw_key` for `endpoint`,
/// terminated with a newline. Written into `buffer`.
pub fn encodeEntry(buffer: []u8, endpoint: []const u8, raw_key: []const u8) ![]const u8 {
